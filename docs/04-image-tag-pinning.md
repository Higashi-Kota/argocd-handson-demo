...
本章は **手元で `docker build + kind load`** を行う流れを学ぶための章です。実運用では以下のような問題があります:

- 個人 PC 依存で再現性が低い
- 複数チームメンバーで image を共有できない
- 別の Kubernetes クラスタ (本番) に同じ image を配れない

この問題を解決するため、**[13 GitHub Actions to GHCR](13-github-actions-ghcr.md)** で CI/CD パイプラインを構築する方法を紹介しています。中核ハンズオンを完了したら、ぜひそちらも実装してみてください。
